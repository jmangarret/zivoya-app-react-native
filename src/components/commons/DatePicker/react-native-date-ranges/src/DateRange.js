...
export default class DateRange extends Component {
  constructor(props){
    super(props);
    const defalutFormat = (!props.mode || props.mode === 'single') ? 'MMMM DD, YYYY' : 'MM/DD/YY';
    this.state = {
  ...
  onDatesChange = (event) => {
    ...
    const defalutFormat = (!this.props.mode || this.props.mode === 'single') ? 'MMMM DD, YYYY' : 'MM/DD/YY';
          ...
          {this.props.mode === 'single' && 
            <View>
              ...
              <TouchableOpacity onPress={this.selectMonthAndDate}>
                <Text style={{ fontSize: 30, color:'white', fontWeight:'bold' }}>{this.toUpperCaseFirst(this.state.clearSingle)}</Text>
              </TouchableOpacity>
            </View>
          }
          {this.props.mode === 'range' && 
            <View>
              ...
              <View style={styles.dateContainer}>
                <Text style={headerDate2}>{this.state.clearStart ? this.state.clearStart : I18n.t('departureDate')}</Text>
                ...
                <Text style={headerDate2}>{this.state.clearEnd ? this.state.clearEnd : I18n.t('returnDate')}</Text>
              </View>
            </View>
          }
        </View>
        {this.state.selectState === 'monthAndDate' &&
          <View style={styles.calendar}>
            ...
            <View style={styles.headActionContainer}>
              <TouchableOpacity style={{ width: 40, height: 20}} onPress={this.previousMonth}>
              <Icon
                    color="black"
                    name="chevron-left"
                    size={20}
              ...
              {/* <Text style={{fontSize:20, color:'black', fontWeight:'bold' }}>{this.state.focusedMonth.format('MMMM YYYY').toUpperCase()}</Text> */}
              <Text style={{fontSize:20, color:'black', fontWeight:'bold' }}>{this.toUpperCaseFirst(this.state.focusedMonth.format('MMMM YYYY'))}</Text> 
              <TouchableOpacity style={{ width: 40, height: 20}} onPress={this.nextMonth}>
              <Icon
                    color="black"
                    name="chevron-right"
                    size={20}
