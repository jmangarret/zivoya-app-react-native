...
import Qs from 'qs';
import { Input, Text } from '../commons';
...
export default class GooglePlacesAutocomplete extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.cleanCache != this.props.cleanCache) {
            this.setState({ text: '' })
        }
    }
                    ...
                    if (responseJSON.status === 'OK') {
                        if (this._isMounted === true) {
                            ...
                            details.address_components.map(component => {
                                dataObject[component.types[0]] = [component.long_name, component.short_name.toLowerCase()]
                            });

                            this.setState({
                                text: dataObject.street_number[1] + " " + dataObject.route[1].toLocaleUpperCase(),
                                listViewDisplayed: false
                            });

                            delete rowData.isLoading;

                            this.props.onPress(rowData, details, dataObject);
                        }
                    } else {
                        this._disableRowLoaders();
    ...
    _onChangeText = (text) => {
        ...
        text = typeof (text.text) != "undefined" ? text.text : this.props.value;
        if (text.length > 1) {
            this._isLoading = true;
        }
    ...
    _getRowLoader() {
        return (
            <ActivityIndicator style={{ top: -10, opacity: true ? 1 : 0 }} size="small" color={"red"} animating />
        );
    }
    ...
    _renderRowData = (rowData) => {
        ...
        return (
            <View style={{ flexDirection: 'row', padding: 0, backgroundColor: 'white', borderBottomColor: "#DBDBDB", borderBottomWidth: 1, height: 50 }} >
                <Icon name="location-pin"
                    top={40}
                    size={20}
                    style={{ paddingLeft: 5, marginRight: 5, marginTop: 15 }}
                />
                <View style={{ flexDirection: 'column' }}>
                    <Text style={defaultStyles.secondaryText} numberOfLines={1} ellipsizeMode="tail">
        ...
        return (
            <ScrollView
                ...
                <TouchableHighlight
                    ...
                    <View style={[this.props.suppressDefaultStyles ? { top: 0, bottom: 10 } : defaultStyles.row, this.props.styles.row, rowData.isPredefinedPlace ? this.props.styles.specialItemRow : {}]}>
    ...
    _renderLoading = () => {
        ...
        if (this._isLoading === true) {
            return (
                <ActivityIndicator style={{ opacity: true ? 1 : 0, flex: 1, flexDirection: 'row', top: 25, right: 5, alignSelf: 'flex-start', position: 'absolute', }} size="small" color={"black"} animating />
            )
        }
    }
    ...
    _getFlatList = () => {
        ...
        if ((this.state.text !== '' || this.props.predefinedPlaces.length || this.props.currentLocation === true) && this.state.listViewDisplayed === true) {
            return (
                <FlatList
                    //style={[this.props.suppressDefaultStyles ? {paddingTop: -10} : defaultStyles.listView, this.props.styles.listView]}
                ...
                {!this.props.textInputHide &&
                    ...
                    >
                        ...
                        <Input
                            ...
                            onFocus={this.props.onFocus ? () => { this._onFocus(); this.props.onFocus(); } : this._onFocus}
